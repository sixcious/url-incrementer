...
var Action = (() => {
   ...
   * @private
   */
  async function incrementDecrementErrorSkip(action, instance, errorSkipRemaining, retrying) {
  ...
  function clear(caller, instance, items) {
    ...
    instance.enabled = instance.multiEnabled = instance.downloadEnabled = instance.autoEnabled = instance.autoPaused = instance.autoRepeat = instance.shuffleURLs = instance.listEnabled = false;
    ...
    switch (instance.toolkitTool) {
      case "crawl":
        ...
        if (chrome.windows && chrome.windows.create) {
          chrome.windows.create({url: chrome.runtime.getURL("/html/popup.html"), type: "popup", width: 600, height: 600}, function(window) {
      ...
      chrome.tabs.executeScript(instance.tabId, {file: "/js/download.js", runAt: "document_end"}, function() {
        ...
        chrome.tabs.executeScript(instance.tabId, {code: code, runAt: "document_end"}, function (results) {
          ...
          if (instance.downloadSubfolder && instance.downloadSubfolderIncrement) {
            instance.downloadSubfolder = instance.downloadSubfolder.replace(/\d+(?!.*\d+)/, function(match) {
